...
def TextToSpeech(Text, func=lambda r=None: True):
    ...
    # Adjust sentence splitting based on detected language
    if language == "bn":  # Bangla
        Data = str(Text).replace("।", ".").split(".")
    else:  # Default (English & other languages)
        Data = str(Text).split(".")
    ...
    if len(Data) > 4 and len(Text) > 250:
        ...
        message = " ".join(Data[0:2]) + ". "  # First part of the text
        time.sleep(1)  # Pause execution for 1 second
        message += random.choice(responses)  # Add the random response after the pause

        TTS(message, func)  # Pass the full message to TTS
    else:
        TTS(Text, func)
...
if __name__ == "__main__":
    while True:
        ...
        print("AssistantVoice:", AssistantVoice)
